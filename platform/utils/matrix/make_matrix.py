...
if __name__ == '__main__':
    ...
    parser.add_argument('paths_checks', type=str, default='../../groups', help='File generated by the script paths_checker.py')
    ...
    paths_checks = args.paths_checks
    ...
    co_dic = {}
    with open(groups_dir+'/matrix/connectivity.txt', 'r') as fd:
        for line in fd.readlines():
            linetab = line.rstrip('\n').split('\t')
    ...
    with open(paths_checks, 'r') as fd:
        for line in fd.readlines():
            linetab = line.rstrip('\n').split('\t')
            from_as = int(linetab[0])
            to_as = int(linetab[1])
