...
When building the mini-Internet, a script called `save_configs.sh` is automatically generated and aims at saving all the routers and switches configuration in a zip file. There is one `save_config.sh` script for each, which is available in the SSH proxy container of the corresponding group. 
