...
def container_setup(config: Topology, directory: Path):
    ...
    os.environ["DOCKERHUB_PREFIX"] = "miniinterneteth/"
    os.environ["DOCKER_TAG"] = "26"
    os.environ["VPN_OBSERVER_SLEEP"] = "30"
        ...
        if isinstance(domain, AS):
            ...
            for name, l2_network in domain.l2_networks.items():
                ...
                for switch in l2_network.switches:
                    ...
                    subnet_ssh_switch = str(IPv4Interface(subnet_sshContainer_groupContainer(group_no, -1, switch.bridge_id, "switch")).ip)
