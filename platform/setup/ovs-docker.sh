...
add_port () {
    ...
    if [ -n "$THROUGHPUT" ]; then
        ...
        echo "echo -n \" -- set interface "${PORTNAME}"_l ingress_policing_burst=100 \" >> groups/throughput.sh " >> groups/delay_throughput.sh
        echo "  ovs-vsctl set interface ${PORTNAME}_l ingress_policing_rate=${THROUGHPUT}" >> groups/restart_container.sh
        echo "  ovs-vsctl set interface ${PORTNAME}_l ingress_policing_burst=100" >> groups/restart_container.sh
    fi
