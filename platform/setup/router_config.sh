...
for ((i=0;i<n_extern_links;i++)); do
    ...
    if [ "${group_as_1}" = "IXP" ] || [ "${group_as_2}" = "IXP" ];then
        ...
        echo " -c 'neighbor "${subnet2%???}" route-map IXP_OUT out'\\" >> "${location}"
        echo " -c 'neighbor "${subnet2%???}" route-map IXP_IN in'\\" >> "${location}"
        echo " -c 'exit'\\" >> "${location}"
        ...
        echo " -c 'ip community-list 1 permit $grp_1:10' \\" >> "${location}"
        echo " -c 'route-map IXP_OUT permit 10'\\" >> "${location}"
        echo " -c 'set community $str_tmp'\\" >> "${location}"
        echo " -c 'match ip address prefix-list OWN_PREFIX'\\" >> "${location}"
        echo " -c 'route-map IXP_OUT permit 20'\\" >> "${location}"
        echo " -c 'set community $str_tmp'\\" >> "${location}"
        echo " -c 'match community 1'\\" >> "${location}"
        echo " -c 'route-map IXP_IN permit 10' \\" >> "${location}"
        ...
        echo " -c 'neighbor "${subnet2%???}" route-map LOCAL_PREF_IN in' \\" >> "${location1}"
        echo " -c 'neighbor "${subnet2%???}" route-map LOCAL_PREF_OUT out' \\" >> "${location1}"
        ...
        if [ $relation_grp_1 == 'Provider' ]; then
            ...
            echo " -c 'route-map LOCAL_PREF_IN permit 10' \\" >> "${location1}"
            echo " -c 'set community $grp_1:10' \\" >> "${location1}"
            echo " -c 'set local-preference 100' \\" >> "${location1}"
            echo " -c 'route-map LOCAL_PREF_OUT permit 5' \\" >> "${location1}"
            echo " -c 'match ip address prefix-list OWN_PREFIX' \\" >> "${location1}"
            echo " -c 'route-map LOCAL_PREF_OUT permit 10' \\" >> "${location1}"
        ...
        echo " -c 'neighbor "${subnet1%???}" route-map LOCAL_PREF_IN in' \\" >> "${location2}"
        echo " -c 'neighbor "${subnet1%???}" route-map LOCAL_PREF_OUT out' \\" >> "${location2}"
        ...
        if [ $relation_grp_2 == 'Provider' ]; then
            ...
            echo " -c 'route-map LOCAL_PREF_IN permit 10' \\" >> "${location2}"
            echo " -c 'set community $grp_2:10' \\" >> "${location2}"
            echo " -c 'set local-preference 100' \\" >> "${location2}"
            echo " -c 'route-map LOCAL_PREF_OUT permit 5' \\" >> "${location2}"
            echo " -c 'match ip address prefix-list OWN_PREFIX' \\" >> "${location2}"
            echo " -c 'route-map LOCAL_PREF_OUT permit 10' \\" >> "${location2}"
