...
for ((k=0;k<group_numbers;k++)); do
    ...
    if [ "${group_as}" != "IXP" ];then
        ...
        docker run -itd --net='none'  --name="${group_number}""_ssh" \
          ...
          --memory=100m --memory-swap=100m --cpus=2 \
    	...
    	for ((l=0;l<n_l2_switches;l++)); do
            ...
            docker run -itd --net='none' --dns="${subnet_dns%/*}" --privileged \
                --memory=100m --memory-swap=100m --cpus=2 --hostname "${sname}" \
        ...
        for ((l=0;l<n_l2_hosts;l++)); do
            ...
            if [[ $hname != vpn* ]]; then
                docker run -itd --net='none' --dns="${subnet_dns%/*}" --privileged \
                    --memory=100m --memory-swap=100m --cpus=2 --hostname "${hname}" \
        ...
        for ((i=0;i<n_routers;i++)); do
            ...
            docker run -itd --net='none'  --dns="${subnet_dns%/*}" \
                ...
                --memory=100m --memory-swap=100m --cpus=2 --hostname "${rname}""_router" \
            ...
            if [ "${property2}" == "host" ];then
                docker run -itd --net='none' --dns="${subnet_dns%/*}"  \
                    ...
                    --memory=100m --memory-swap=100m --cpus=2 --hostname "${rname}""_host" \
